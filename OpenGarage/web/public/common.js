...
export async function garageFetch(path, params = []) {
    return await fetch(garageRoot + path + "?" + params.join("&"));
}
...
export function initalize(root, is_ap) {
    ...
    themeSelector.addEventListener("input", () => {
        localStorage.setItem("darkMode", themeSelector.checked ? "true" : "false");
        document.documentElement.setAttribute("data-theme", themeSelector.checked ? "dark" : "light");
    });
    ...
    versionNumber = document.createElement("span");
