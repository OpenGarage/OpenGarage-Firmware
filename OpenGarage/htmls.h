...
$('#btn_click').html(jd.door>=2?'Click':(jd.door?'Close Door':'Open Door')).button('refresh');
...
<input type='radio' name='secv' id='secv0' value=0 class='green' checked onclick='update_secv()'><label for='secv0'>None</label>
</fieldset>
</td></tr>
...
if(jd.has_swrx){
$('#secv').show();
$('input[name="secv"]').prop('checked', false);
$('label[for="secv0"],label[for="secv1"],label[for="secv2"]').removeClass('ui-btn-active');
$('#secv' + (jd.secv || 0)).prop('checked', true);
$('input[name="secv"]').checkboxradio('refresh');
}
update_secv();
